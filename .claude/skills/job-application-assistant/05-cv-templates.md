...
**Earlier Career section note wording:** keep the note to the plain `Details available on request.` only. Do not include the fuller "Undated per Canadian age-discrimination convention..." explanation on an actual tailored CV - that's the internal rationale (documented in `09-canada-conventions.md`), not something a recruiter needs to read on the page itself. This wording previously leaked from the master reference into a tailored CV verbatim; the master (`main_example.tex`) now uses the same plain wording specifically to prevent that recurring.

**Earlier Career note placement: same line as the section heading, not a separate line below it.** Put it flush-right on the `\section{Earlier Career}` line itself, not as a separate `\textit{}` line underneath (that costs a full extra line of vertical space for four words):
```latex
\section{Earlier Career\hfill{\color{black}\normalfont\itshape\small Details available on request.}}
```
The `\hfill` inside the `\section{}` argument pushes the note to the right edge of the same heading line; the local `{\color{black}\normalfont\itshape\small ...}` group resets away from the section's blue/bold `\sectionstyle` override so the note renders as small italic black text, not blue bold. Drop the separate `\vspace{1pt}\textit{Details available on request.}\vspace{3pt}` lines entirely when using this pattern - the heading line already carries the note.
