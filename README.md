...
## 🇨🇦 This is the Canadian fork

A community adaptation of [MadsLorentzen/ai-job-search](https://github.com/MadsLorentzen/ai-job-search) for the Canadian market. Everything upstream does, it still does — this fork adds:
...
| **`eluta-search`** | [Eluta.ca](https://www.eluta.ca) — Canadian job search engine behind the "Canada's Top 100 Employers" project; English-only |
...
| **Bilingual search** | `jobbank-ca-search`, `gcjobs-search`, and `talent-com-search` take `--lang en\|fr` and search the French listings, which surface postings the English side does not (`eluta-search` is English-only — no French path was found) |

All four new portals are **public data, no authentication, no API key, and zero runtime dependencies** — plain `bun`.
...
> **Naming:** upstream's `jobbank-search` is Denmark's `jobbank.dk`. Canada's Job Bank is `jobbank-ca-search`. The Canadian one always carries the `-ca` suffix.
...
A structured workflow that turns Claude Code into a full-stack job application assistant. The core workflow (self-profiling, fit evaluation, and the drafter-reviewer application pipeline) is **language- and country-agnostic**. In this fork the job portal search skills target **Canada** — Job Bank / Guichet-Emplois and GC Jobs — alongside the country-agnostic LinkedIn and freehire skills. Upstream's Danish portals ship disabled, and the same pattern can be pointed at any other board with `/add-portal`.
...
```powershell
$tools = @("jobbank-ca-search", "gcjobs-search", "talent-com-search", "eluta-search", "linkedin-search", "freehire-search")
...
```bash
for tool in jobbank-ca-search gcjobs-search talent-com-search eluta-search linkedin-search freehire-search; do
  (cd .agents/skills/$tool/cli && bun install)
done
```

All six are zero-runtime-dependency, so this step is optional — `bun install` only pulls
...
`jobnet-search`) to that list only if you re-enable them; they do have runtime dependencies.
...
```bash
/apply https://www.jobbank.gc.ca/jobsearch/jobposting/49974823
```
...
│   ├── eluta-search/                  # 🇨🇦 Eluta.ca (Canada, EN only, Top 100 Employers)
...
│   └── jobnet-search/                 # Jobnet.dk (Denmark, government portal, disabled)
├── cv/
...
This fork's four Canadian CLI tools in `.agents/skills/` (`jobbank-ca-search`, `gcjobs-search`, `talent-com-search`, `eluta-search`) — along with upstream's four Danish demos — show the pattern for building a job-portal integration for a specific market. Each carries a `url-reference.md` documenting its endpoints, quirks, and the parsing anchors to repair when the site changes; the Canadian ones also record the approaches that turned out **not** to work, so nobody rediscovers them.
...
Alongside the two Canadian portals, the repo ships two **country-agnostic** skills that need no adaptation:
